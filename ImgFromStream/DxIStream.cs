...
namespace Claron.WIF.Dx {

  public unsafe struct IStreamImpl : IStream.Interface {
    
    private Stream _stream;

    public static IStreamImpl* Create(Stream stream) {
      IStreamImpl* result = (IStreamImpl*)NativeMemory.Alloc((nuint)sizeof(IStreamImpl*));
      *result = new IStreamImpl(stream);
      return result;
    }

    internal IStreamImpl(Stream stream) {
      _stream = stream;
    }
